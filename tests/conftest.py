...
from python_demo import PythonDemo, AsyncPythonDemo, DefaultAioHttpClient
from python_demo._utils import is_dict
...
logging.getLogger("python_demo").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[PythonDemo]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with PythonDemo(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncPythonDemo]:
